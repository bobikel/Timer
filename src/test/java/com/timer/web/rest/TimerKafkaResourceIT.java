...
import com.timer.IntegrationTest;
import com.timer.config.EmbeddedKafka;
import com.timer.config.KafkaSseConsumer;
import com.timer.config.KafkaSseProducer;
...
class TimerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/timer-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/timer-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/timer-kafka/unregister"));
                return;
            }
        }
